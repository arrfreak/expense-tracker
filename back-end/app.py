...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://<POSTGRES_USER>:<POSTGRES_PWD>@localhost/postgres'
...
def get_data():
    ...
    sumMonth = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumMonth')).filter(expenses.date >= startDate).filter(expenses.date <= endDate)

    for total in sumMonth:
        sumMonth = total.sumMonth
        if sumMonth is None:
            sumMonth = 0
        else:
            sumMonth = round(total.sumMonth, 2)
    results = [
        {
            "id": Expense.id,
    ...
    return {"Total": sumMonth, "Count": len(results), "Expenses": results}
...
def getSum():
    ...
    qryMonthWise = "SELECT extract(month from date), extract(year from date) as yyyy, SUM(price) FROM public.expenses WHERE date > '2018-12-31' GROUP by 1, 2 ORDER by 2, 1"
    ...
    monthResults = [

        {
            ...
            "month/year": str("%02d" % (int(row[0])),) + "/" + str(int(row[1])),
            "total": round(row[2], 2)
...
# names = [row[0] for row in data]
    return{"CurrentCategoryTotal": catResults, "CurrentCardTotal": cardResults, "AllMonthTotal": monthResults}
...
def get_Monthdata():
    ...
    sumCiti = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumCiti')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'CITI CREDIT CARD')

    # Get Bofa Credit Card Total
    sumBofaCr = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumBofaCr')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'BOFA CREDIT CARD')
    ...
    sumMarriottChase = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumMarriottChase')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'MARRIOTT CREDIT CARD')

    # Get Chase Credit Card Total
    sumChaseCr = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumChaseCr')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'CHASE CREDIT CARD')

    # Get Discover Credit Card Total
    sumDiscoverCr = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumDiscoverCr')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'DISCOVER CREDIT CARD')

    # Get DCU Checking Total
    sumDCUChk = expenses.query.with_entities(func.sum(expenses.price).label(
        'sumDCUChk')).filter(expenses.date >= startDate).filter(expenses.date <= endDate).filter(expenses.card == 'DCU CHECKING')
